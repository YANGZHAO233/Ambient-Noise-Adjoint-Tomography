...
if($ifault==1) {
   $dir0 = "/home/kai/specfem3d/EXAMPLES/South_Cal_Kai2016";
...
$Rlab = "$sta.$net.${chan}R";
$Zlab = "$sta.$net.${chan}Z";
$Tsyn = `ls ${opt_s}/$net.$sta.${chan}T*`; chomp($Tsyn);
$Rsyn = `ls ${opt_s}/$net.$sta.${chan}R*`; chomp($Rsyn);
$Zsyn = `ls ${opt_s}/$net.$sta.${chan}Z*`; chomp($Zsyn);
...
$Trecon = "${opt_c}/$sta.$net.${chan}T${suffix}";
$Rrecon = "${opt_c}/$sta.$net.${chan}R${suffix}";
$Zrecon = "${opt_c}/$sta.$net.${chan}Z${suffix}";
...
`psxy temp.txt $bounds $proj -Sa${cmt_scale}/$cmtfont -G255/0/0 -O -K >> $ps_file`;  # CMT source
`rm temp.txt`;
...
}} else {`pwd |psxy -X-2 -Y4.5 $proj $bounds -N -K -O $tick >> $ps_file`;}
...
else {`pwd |psxy -Y$dY $proj $bounds -N -K -O $tick2 >> $ps_file`;}
