...
if (@ARGV != 8) {die("Usage: combine_3_adj_src.pl chan DIR_1 DIR_2 DIR_3 DIR_NEW type1(cc,ik,mtm) type2 type3\n");}
...
for ($i=1; $i<@sta; $i++){
  ...
  @files = glob("$dir1/$sta.$nt.${chan}?.*adj");
  if (@files == 0) {print("Check if $dir1/$sta.$nt.${chan}?.*adj exist or not\n");}
}
...
for ($i=1; $i<@sta; $i++){
  ...
  @files = glob("$dir2/$sta.$nt.${chan}?.*adj");
  if (@files == 0) {print("Check if $dir2/$sta.$nt.${chan}?.*adj exist or not\n");}
}
...
for ($i=1; $i<@sta; $i++){
  ...
  @files = glob("$dir3/$sta.$nt.${chan}?.*adj");
  if (@files == 0) {print("Check if $dir3/$sta.$nt.${chan}?.*adj exist or not\n");}
}
...
#foreach $sta ("TCF") {
  ...
  foreach $comp ("${chan}E","${chan}N","${chan}Z") {
