...
if ($iplot == 1) {
  ...
  print CSH "cp OUTPUT_FILES/*recon* ${plot_recon_dir}\n";
...
if ($iadj == 1) {
  ...
  $tfiles = "OUTPUT_FILES/*recon.sac";
}
$adj_dir = "ADJOINT_SOURCES";
print CSH "\\rm -rf ${adj_dir}\n mkdir ${adj_dir}\n";
print CSH "./prepare_adj_src.pl -m $cmtfile -z $chan -s PLOTS/$stafile1 -o ${adj_dir} $tfiles\n";
