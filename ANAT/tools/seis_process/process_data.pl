...
sub Usage{
 ...
 Notice:
  ...
  3. The displacement outputsoaxf after -i option are in the unit of meters
...
Revised:
  add -n for normalizetion of trace
  add -v to obtain EGF from derivative of CCF using ambient noise data
  Kai Wang, updated in Feb 2016
END
  exit(1);
}
$ENV{'SACAUX'}='/work/ose-zhaoy/software/sac/aux';
if (@ARGV == 0) { Usage(); }

if (!getopts('m:a:l:t:fiR:px:d:s:P:vnT:A:y:c')) {die('Check input arguments\n');}
...
#    print SAC "ch allt %tmp% iztype io\n";
  ...
  if ($opt_v){# deviate records
     print "    Deviate record ...\n";
     print SAC "dif\n";
     print SAC "mul -1\n";
     print SAC "w $outfile\n";}
  ...
  if ($opt_l){  # cut record
    ...
    print SAC "cuterr usebe\n cut $lmin $lmax\n r $outfile\n rmean;rtrend;taper;\n w $outfile\n";
    print SAC "cuterr fillz\n";
    print SAC "cut $lmin $lmax\n";
...
#    print SAC "rtrend\n rmean\n taper width $opt_T\n";
    print SAC "rmean;rtrend;taper;\n";
    printf SAC ("bp n %4d p $pass cor %12.6f %12.6f\n",$poles,$f1,$f2);
#    printf SAC " rtrend\n rmean\n taper width $opt_T\n";}
    printf SAC "rmean;rtrend;\n w over \n";}
  ...
  if ($opt_n) { # normalize trace
    print "    Normalize trace\n";
    (undef,$tmp_dist)=split(" ",`$saclst dist f $outfile`);
    $tmp_r0=$tmp_dist/1.8;
    $tmp_r1=$tmp_dist/4.5;
    print "signal window: $tmp_r1 $tmp_r0\n";
    print SAC "cut $tmp_r1 $tmp_r0 \n";
    print SAC "r $outfile\n";
    print SAC "abs\n";
    print SAC "evaluate to tmp &1,depmax\n";
    print SAC "cut off \n";
    print SAC "r $outfile\n";
    print SAC "div %tmp\n";}
