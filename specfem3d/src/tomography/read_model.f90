...
! reads in current transverse isotropic model: vpv.. & vsv.. & eta & rho
  ...
  read(IIN) ival0 !nspec
  if (ival0 /= nspec) call exit_mpi(myrank,'Error invalid nspec value in external_mesh.bin')
  read(IIN) ival !nglob
  ...
  !if (myrank==0) then
   ! write(*,*)'GLLX,GLLY,GLLZ,ival,ival0,nglob,nspec:',NGLLX,NGLLY,NGLLZ,ival,ival0,nglob,nspec 
  !endif
